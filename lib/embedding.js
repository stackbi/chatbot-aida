...
export function findSimilarChunks(queryEmbedding, chunkEntries, topK = 4) {
  ...
  // Norme de la requête : calculée UNE seule fois hors de la boucle (elle est
  // identique pour tous les chunks) au lieu d'être recalculée à chaque paire.
  let normA = 0;
  ...
  for (const entry of chunkEntries) {
    ...
    if (b.length !== queryEmbedding.length) continue;

    let dot = 0;
    let normB = 0;
    for (let i = 0; i < b.length; i++) {
      dot += queryEmbedding[i] * b[i];
      normB += b[i] * b[i];
    }
