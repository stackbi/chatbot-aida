...
let cacheMtime = 0;
let cacheSize = 0; // taille du fichier à la dernière lecture (double vérification du cache)
const CACHE_TTL = 2000; // 2 secondes entre chaque relecture disque

// Lit l'état du fichier (mtime + taille) pour la validation du cache.
// La taille est comparée EN PLUS du mtime : sur certains systèmes de fichiers
// (volumes Docker/Render à granularité grossière), deux écritures rapides
// peuvent partager le même timestamp → la taille détecte quand même le
// changement et évite de servir des données périmées.
function statStoreFile() {
  try {
    const st = fs.statSync(DATA_FILE);
    return { mtime: st.mtimeMs, size: st.size };
  } catch {
    return null; // fichier absent/illisible
  }
}
...
// État au démarrage : store.json existait-il AVANT qu'on le crée/écrive ?
// Si non ET que le marqueur n'a pas survécu, c'est très probablement le
// PREMIER démarrage (rien à perdre encore) — distinct d'un stockage éphémère
// qui aurait déjà effacé des données existantes.
const HAD_STORE_AT_BOOT = fs.existsSync(DATA_FILE);
...
function readStore() {
  ...
  // Optimisation : si le FICHIER n'a pas changé depuis la dernière lecture
  // (mtime identique), le contenu en cache est toujours à jour → on l'utilise
  // sans re-parser le JSON (coûteux sur les gros store.json avec embeddings).
  if (storeCache) {
    const st = statStoreFile();
    // Fichier inchangé (même mtime ET même taille) → le cache est à jour
    if (st && st.mtime === cacheMtime && st.size === cacheSize) {
      cacheTimestamp = now; // prolonge le cache sans relecture disque
      return storeCache;
    }
  }
  try {
    ...
    const st = statStoreFile();
    cacheMtime = st ? st.mtime : 0;
    cacheSize = st ? st.size : 0;
...
function writeStore(store) {
  ...
  // Invalide le cache après écriture (le mtime du nouveau fichier sera relu)
  storeCache = null;
  const st = statStoreFile();
  cacheMtime = st ? st.mtime : 0;
  cacheSize = st ? st.size : 0;
...
/**
 * État détaillé du stockage pour l'admin — permet une bannière justifiée :
 * - persistent : le marqueur a survécu au redémarrage précédent (volume OK)
 * - firstBoot : premier démarrage (ni marqueur ni store.json) → rien à perdre
 *   encore, la persistance sera confirmée au prochain redémarrage
 * - hasData : un store.json existe (une clé/document a déjà été enregistré)
 * - hasEnvApiKeys : au moins une clé API est fournie par variable
 *   d'environnement → elle survit aux redémarrages PAR NATURE
 */
export function getStorageStatus() {
  ...
  return {
    ...
    // Si le marqueur n'a pas survécu mais qu'aucun store.json n'existait au
    // démarrage, c'est le premier lancement : l'avertissement de perte n'a pas
    // lieu d'être (aucune donnée n'a encore pu être perdue).
    firstBoot: !storagePersistent && !HAD_STORE_AT_BOOT,
    // Des données existaient-elles AVANT ce démarrage ? (Évalué sur l'état
    // capturé au boot, PAS sur l'existence actuelle du fichier : ensureStore
    // crée store.json au premier accès, ce qui rendrait ce test toujours vrai.)
    hasData: HAD_STORE_AT_BOOT,
    hasEnvApiKeys: envKeys.some((k) => !!k)
  };
}
